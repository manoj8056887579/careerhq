...
export const metadata: Metadata = {
  title: "Internships in India | Career HQ",
...
async function getModulesData() {
  try {
    ...
    const [modulesRes, categoriesRes] = await Promise.all([
      fetch(
        `${baseUrl}/api/modules?moduleType=internship-india&published=true`,
        {
          cache: "no-store",
        }
      ),
      fetch(`${baseUrl}/api/modules/categories?moduleType=internship-india`, {
        cache: "no-store",
      }),
    ]);
...
export default async function InternshipIndiaPage() {
