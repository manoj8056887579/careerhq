...
async function importIreland() {
  try {
    const rawData = fs.readFileSync(
      path.join(process.cwd(), "data", "ireland_updated.json"),
      "utf-8"
    );
    ...
    let country = await Country.findOne({ name: "Ireland" });
    if (!country) {
      country = new Country({
        name: "Ireland",
        code: "IE",
        slug: "ireland",
        description: "Ireland is a leading destination for higher education",
        currency: "EUR",
        language: "English",
        timezone: "UTC+0",
        published: true,
      ...
      await country.save();
      console.log("✓ Created country: Ireland");
    } else {
      console.log("✓ Country already exists: Ireland");
    }
    ...
    for (const [, uniData] of uniqueUniversities) {
      try {
        ...
        const location = `${uniData.Location}, Ireland`;
        ...
        university = new University({
          ...
          tags: ["Ireland", "Research University"],
        });
