...
async function importCanada() {
  try {
    const rawData = fs.readFileSync(
      path.join(process.cwd(), "data", "canada_updated.json"),
      "utf-8"
    );
    ...
    let country = await Country.findOne({ name: "Canada" });
    if (!country) {
      country = new Country({
        name: "Canada",
        code: "CA",
        slug: "canada",
        description: "Canada is a leading destination for higher education",
        currency: "CAD",
        language: "English",
        timezone: "UTC-5",
        published: true,
      ...
      await country.save();
      console.log("✓ Created country: Canada");
    } else {
      console.log("✓ Country already exists: Canada");
    }
    ...
    for (const [, uniData] of uniqueUniversities) {
      try {
        ...
        const location = `${uniData.Location}, Canada`;
        ...
        university = new University({
          ...
          tags: ["Canada", "Research University"],
        });
